...
class OrderItemTest < ActiveSupport::TestCase
  def setup
    ...
    @product = Product.create!(provider: @provider, name: "P", price: 100, stock: 5)
    @order = Order.create!(store: @store, total: 0)
  end

  test "validaciones de cantidad" do
    item = @order.order_items.build(product: @product, quantity: 0, unit_price: 100, line_total: 0)
  ...
  test "line_total debe ser unit_price x quantity" do
    item = @order.order_items.build(product: @product, quantity: 3, unit_price: 100, line_total: 999)
